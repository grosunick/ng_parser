...
class ProxyService(ABC):
    ...
    на усмотрение реализации. Команды получают только готовый HttpClient и про HttpProxy не знают.
    """
    ...
    async def report_bad(self, client: HttpClient, reason: Exception) -> None:
        ...
        какой HttpProxy с ним связан, и решает — пометить, выкинуть, поставить в cooldown."""
        ...
