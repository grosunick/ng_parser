"""Тесты для ng_parser.algorithm.AsyncParser."""
...
def test_run_uses_passed_url():
    ...
    async def _():
        parser = make_async_parser(client)
...
def test_zero_max_workers_raises():
    ...
    with pytest.raises(ValueError):
        AsyncParser(max_workers=0, client=client, repository=repo)
    asyncio.run(client.close())
...
def test_negative_max_workers_raises():
    ...
    repo = ListRepository()
    with pytest.raises(ValueError):
        AsyncParser(max_workers=-1, client=client, repository=repo)
    asyncio.run(client.close())


def test_constructor_requires_client():
    with pytest.raises(TypeError, match="client"):
        AsyncParser(max_workers=1, repository=ListRepository())  # type: ignore[call-arg]


def test_constructor_requires_repository():
    client = make_test_client(_ok_transport())
    with pytest.raises(TypeError, match="repository"):
        AsyncParser(max_workers=1, client=client)  # type: ignore[call-arg]
    asyncio.run(client.close())
...
def test_end_to_end_batch_all_urls_fetched():
    ...
    async def _():
        parser = make_async_parser(client, workers=5)
